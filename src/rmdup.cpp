...
//    memset(dataContent,0,sizeof(uint64_t*)*contentSize);
            ...
            )
    {
        if (count[i]
            > 1)
        {
            dataContent[contentIdx]
                    = new uint64_t[count[i]];
            contentIdx++;
        }
    }
...
//            cout<<get_local_time()<<"\tin forth scanning:\t"<<i<<endl;
            ...
            > 1)
        {
            ...
            dataContent[multiIdx[idx]][curCount[multiIdx[idx]]]
                    = data[i];
            curCount[multiIdx[idx]]++;
            if (curCount[multiIdx[idx]]
                == count[idx])
            {
...
//                string curStr;
                        ...
                        )
                {
                    uint64_t
                            curDa
                            = dataContent[multiIdx[idx]][j];
//                    curStr=md52string(dataContent[multiIdx[idx]][j]);
                    if (exists.find(curDa)
                        == exists.end())
                    {
                        exists.insert(curDa);
                    }
...
//    float high=1;
    ...
    {
        cerr
                << "Error:code error, please contact with "
                << AUTHOREMAIL
                << endl;
        exit(1);
    }
