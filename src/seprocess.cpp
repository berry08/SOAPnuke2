    ...
    if(gp.cleanOutSplit>0){
            ...
            if (subThreadAllDone) {
                ...
                for (int cycle = cur_cat_cycle; cycle < ready_cycles; cycle++) {
                    for (int i = 0; i < gp.threads_num; i++) {
                        ...
                        ostringstream readyCatR1CleanFile;
                        ...
                        if(fileReadsNum<=gp.cleanOutSplit-lastUncompleteFileReadsNumber){
                            ...
                            extractReadsToFile(cycle, i,readyCatR1Files,fileReadsNum,lastUncompleteFileReadsNumber, "head",
                                               outputFileIndex,
                ...
                if(readyCatR1Files.size()>0){
                    extractReadsToFile(lastCycle, lastI,readyCatR1Files,lastfileReadsNum,lastUncompleteFileReadsNumber, "head",
                                       outputFileIndex,
        ...
        while (1) {  //merge small files by input order
            ...
            if (subThreadAllDone) {
                ...
                string readyR1SmallFiles;
                string readyR1TrimSmallFiles;
...
//                    }
                ...
                if (!gp.trim_fq1.empty()) {
                    ...
                    thread t1=thread(bind(&seProcess::run_cmd,this,mergeShell1));
                    t1.join();
                    mergeShell1="rm "+readyR1TrimSmallFiles;
                    t1=thread(bind(&seProcess::run_cmd,this,mergeShell1));
                    t1.join();
                }
                if (!gp.clean_fq1.empty()) {
...
void seProcess::extractReadsToFile(int cycle,int thread_index,vector<string> r1,int fileReadsNum,int& reads_number,string position,int& output_index,bool gzFormat,bool done){
    ostringstream outFile1;
